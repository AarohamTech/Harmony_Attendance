...
async function initDatabase() {
  ...
  try {
    ...
    // 1. Ensure Default Office Location (Padalkar Colony: 16.740572, 74.246919, 200m)
    await client.query(`
      UPDATE office_locations
      SET office_name = 'Padalkar Colony',
          address = 'Kasaba Bawada, Kolhapur, Maharashtra 416006',
          latitude = 16.740572,
          longitude = 74.246919,
          allowed_radius = 200
      WHERE office_id = 1 OR office_name LIKE '%Silicon%' OR office_name LIKE '%Head Office%' OR allowed_radius = 100
    `).catch(() => {});
    ...
    if (officeCheck.rows.length === 0) {
      const officeRes = await client.query(`
        ...
        VALUES ('Padalkar Colony', 'Kasaba Bawada, Kolhapur, Maharashtra 416006', 16.740572, 74.246919, 200)
        RETURNING office_id
      `);
